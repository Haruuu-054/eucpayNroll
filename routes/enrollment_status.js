...
function enrollmentstatus(supabase, logger) {
  ...
  router.get("/", async (req, res) => {
    try {
      // Step 1: Get active enrollment period with semester
      ...
      if (periodError) {
        logger.error("Error fetching active period: " + periodError.message);
        return res.status(500).json({ error: "Failed to fetch active enrollment period" });
      }
      ...
      // Step 2: Fetch pending enrollments for active semester
      const { data: enrollments, error: enrollmentsError } = await supabase
        .from("enrollments")
        .select("student_id, status, created_at, scheme_id, program_id")
        .eq("status", "Pending")
        .eq("semester_id", activePeriod.semester_id);
       
      if (enrollmentsError) {
        logger.error("Error fetching pending enrollments: " + enrollmentsError.message);
      ...
      if (schemesError) {
        ...
        return res.status(500).json({ error: "Failed to fetch tuition schemes" });
      }
...
function enrolledcounts(supabase, logger) {
  ...
  router.get("/", async (req, res) => {
    try {
      // Get active enrollment period with semester
      const { data: activePeriod, error: periodError } = await supabase
        .from("enrollment_periods")
        .select("semester_id")
        .eq("is_active", true)
      ...
      // Fetch enrolled count for active semester
...
function pendingcounts(supabase, logger) {
  ...
  router.get("/", async (req, res) => {
    try {
      ...
      // Fetch pending count for active semester
      const { count, error } = await supabase
        .from("enrollments")
        .select("*", { count: "exact", head: true })
        .eq("status", "Pending")
      ...
      if (error) {
        logger.error("Error in pending students endpoint: " + error.message);
        return res.status(500).json({ error: "Internal server error" });
      }

      res.json({ count });
    } catch (err) {
      logger.error("Error in pending students endpoint: " + err.message);
      res.status(500).json({ error: "Internal server error" });
    }
  });
